...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- ELEMENTOS MÓVILES ---
    const cartFloatingButton = document.getElementById('cart-floating-button');
    ...
    function renderCarrito() {
        ...
        updateCartFloatingButton(totalUnidades);
    ...
    // --- FUNCIONES MÓVILES ---
    function updateCartFloatingButton(totalUnidades) {
        if (cartCount) {
    ...
    function getPaymentMethodsUrl(bandera) {
        const nombreNormalizado = bandera.toLowerCase().replace(/\s+/g, '');
        if (nombreNormalizado.includes('coto')) return 'https://www.coto.com.ar/descuentos/index.asp';
        if (nombreNormalizado.includes('carrefour')) return 'https://www.carrefour.com.ar/descuentos-bancarios';
        if (nombreNormalizado.includes('gallega')) return 'https://www.lagallega.com.ar/Beneficios.asp';
        if (nombreNormalizado.includes('libertad')) return 'https://www.hiperlibertad.com.ar/institucional/medios-de-pago';
        if (nombreNormalizado.includes('jumbo')) return 'https://www.jumbo.com.ar/descuentos-del-dia';
        return null; // Si no hay URL específica
    }
    ...
    function renderResultados(data, optimizacionData) {
        ...
        data.comparativa.forEach((res, index) => {
            ...
            const paymentUrl = getPaymentMethodsUrl(res.bandera);
            html += `
                <div class="resultado-card ${index === 0 ? 'mejor-opcion' : ''}" data-bandera="${res.bandera}">
                    <div class="card-logo-container" data-target="detalle-${index}">
                        ${paymentUrl ? `<a href="${paymentUrl}" target="_blank" class="payment-methods-button" title="Ver formas de pago y promociones"><span class="payment-text">Formas de Pago</span></a>` : ''}
    ...
    pageContent.addEventListener('click', async e => {
        ...
        if (backButton) {
            showCategoriasView();
            // Reactivar el botón de comparar si hay productos en el carrito
            if (carrito.length > 0) {
                compareButton.disabled = false;
            }
        }
    ...
    // Cerrar carrito móvil con tecla Escape
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && isMobileView() && cartSidebar && cartSidebar.classList.contains('mobile-open')) {
            closeMobileCart();
        }
    });
    ...
    window.addEventListener('resize', () => {
        if (!isMobileView() && cartSidebar && cartSidebar.classList.contains('mobile-open')) {
            closeMobileCart();
        }
    });
